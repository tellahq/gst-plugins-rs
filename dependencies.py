...
PARSER.add_argument(
    "--features", action="store_true", help="Get list of features to activate"
)
...
class CargoAnalyzer:
    ...
    def extract_version(self, feature_name):
        ...
        version_part = feature_name[verindex:]
        if "_" not in version_part:
            return None

        try:
            (majver, minver) = version_part.split("_")
        ...
        for name in opts.plugins:
            ...
            with crate_path.open("rb") as f:
                ...
                if opts.features:
                    ...
                    try:
                        requires = data["package"]["metadata"]["capi"]["pkg_config"][
                            "requires_private"
                        ]
                    except KeyError:
                        continue
